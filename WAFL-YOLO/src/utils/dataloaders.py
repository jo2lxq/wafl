...
from torch.utils.data import DataLoader, Dataset, dataloader, distributed, Subset
...
from utils.generate_iid import generate_iid
from utils.generate_noniid import generate_noniid
...
def create_dataloader_self(path,
                      imgsz,
                      batch_size,
                      stride,
                      single_cls=False,
...
def create_dataloader(path,
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        ...
        dataset_filter = LoadImagesAndLabels(
            path,
            imgsz,
            batch_size,
    ...
    generator = torch.Generator()
    generator.manual_seed(6148914691236517205 + RANK)
    dl_filter = loader(dataset_filter,
                batch_size=batch_size,
                shuffle=False,
                num_workers=0,
                sampler=sampler,
    ...
    if mode == "train":
        ...
        loader =  DataLoader(dataset, batch_size, sampler=sampler_val, 
                             drop_last=False, collate_fn=LoadImagesAndLabels.collate_fn4 if quad else LoadImagesAndLabels.collate_fn, num_workers=nw)
