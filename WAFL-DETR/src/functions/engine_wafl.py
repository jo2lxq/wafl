...
def train_one_epoch(models: list[torch.nn.Module], criterion: torch.nn.Module,
    ...
    for i in range(len(models)):
        ...
        for samples, targets in loader:
            ...
            losses.backward()
            if max_norm > 0:
                torch.nn.utils.clip_grad_norm_(models[i].parameters(), max_norm)
            optimizers[i].step()
...
def evaluate(models, criterion, postprocessors, data_loader, base_ds, device, output_dir, detail_log):
    ...
    for i in range(len(models)):
        ...
        coco_evaluators.append(coco_evaluator)
    return all_stats, coco_evaluators