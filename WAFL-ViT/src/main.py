...
meant_file_path = os.path.join(mean_and_std_path, "test_mean.pt") ## IIDでもNonIIDでもtest用は同じ
stdt_file_path = os.path.join(mean_and_std_path, "test_std.pt") ## IIDでもNonIIDでもtest用は同じ
...
if (os.path.exists(meant_file_path)) and (os.path.exists(stdt_file_path)): # Test dataの平均と分散のファイルがある場合
    ...
    if useGPUinTrans:
        train_transform = transforms.Compose(
            [
                ...
                transforms.ConvertImageDtype(torch.float32),
                transforms.Normalize(mean=tuple(mean), std=tuple(std)), # 各ノードの画素値の平均、分散をもとに正規化
...
if useGPUinTrans:
    testloader = DataLoader(
        test_data, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=False, worker_init_fn = seed_worker, generator = g
    )
else:
    testloader = DataLoader(
        test_data, batch_size=batch_size, shuffle=False, num_workers=50, pin_memory=True, worker_init_fn = seed_worker, generator = g
    )
...
nets = [select_net(model_name, len(classes), n_middle).to(device) for i in range(n_node)] # model_nameに応じたモデルをとってくる。
...
if __name__ == "__main__":
    ...
    for epoch in range(
        ...
        model_exchange(nets, model_name, contact, fl_coefficiency) # model_nameで指定したモデルで接触したノードとモデル合成を行う。
