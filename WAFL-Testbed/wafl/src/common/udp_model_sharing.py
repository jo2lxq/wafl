...
class UDPModelSharing:
    ...
    def send_model(self, model_data: bytes, target_ip: str, target_port: int) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Sent model ({len(model_data)} bytes) to {target_ip}:{target_port}")
    ...
    def get_survival_rate(self) -> float:
        """Calculate model survival rate (successful transmissions / total attempts)."""
        total = self.stats["sent_models"] + self.stats["sent_failed"]
        if total == 0:
            return 1.0
        return self.stats["sent_models"] / total
        ...
        while self.running:
            try:
                ...
                if source_key not in incoming_models:
                    incoming_models[source_key] = {"chunks": {}, "meta": {}, "last_update": time.time()}
                ...
                if chunk_idx not in model_state["chunks"]:
                    ...
                    model_state["meta"][chunk_idx] = {"total": total_chunks, "len": original_len}
    ...
    def _reassemble_model(self, state):
        try:
            ...
            for i in sorted_chunk_indices:
                ...
                self.stats["fec_recovery_success"] += 1

            self.stats["received_models"] += 1
            self.logger.info(f"Successfully reassembled model ({len(full_data)} bytes)")
            return full_data
        except Exception as e:
