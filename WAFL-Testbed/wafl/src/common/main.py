    ...
    def save_traffic_metrics(csv_path: str, pcap_path: str, epoch_number: int, neighbours: int) -> None:
        ...
        try:
            ...
            for pkt in packet_list:
                if (not IP in pkt) or \
                    datetime.datetime.fromtimestamp(float(pkt.time)) < \
                    NetworkTrafficUtils.last_timestamp or \
                    datetime.datetime.fromtimestamp(float(pkt.time)) > \
                    next_last_timestamp:
                    continue
            ...
            with open(csv_path, "a", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(
                    [f"{epoch_number}", f"{inbound}", \
                    f"{outbound}", f"{neighbours}"]
                )
...
class CTRL_TCP:
    ...
    def __init__(self, config_path: str):
        ...
        # Initialize the Network Traffic Utils Class.
        NetworkTrafficUtils(self.addr, self.p2p_port)
            ...
            if main_command == "KILL":
                ...
                try:
                    self.traffic_thread = threading.Thread(
                        ...
                        daemon=False,
                        args=(NetworkTrafficUtils.last_epoch_string, "KILL",)
                    )
                ...
                try:
                    self.traffic_thread = threading.Thread(
                        target=self.model_learning.network_traffic_thread,
                        daemon=False,
                        args=(five_digit_number_str,sub_command,)
                    )
                    self.traffic_thread.start()
                except Exception as e:
                    self.logger.error(f"Failed to start traffic thread: {e}")
